...
void penalty_kick::calculate() {
  ...
  //パラメータ計算
  double b = 1;
  double a = (target_y - ball_y) / (target_x - ball_x);
  ...
  kick_y = (ball_y < 0) ? -sqrt((500 * 500) - (kick_x - ball_x) * (kick_x - ball_x)) + ball_y
                        : (sqrt((500 * 500) - (kick_x - ball_x) * (kick_x - ball_x)) + ball_y);
