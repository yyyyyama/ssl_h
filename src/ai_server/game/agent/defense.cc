	...
	namespace game{
		namespace action{
			...
			std::vector<std::shared_ptr<action::base>> defens::execute(){
			  	...
			  	//キーパー用のaction
				  auto keeper = std::make_shared<action::move>(keeper_id_);
					
				  //壁用のaction
					std::vector<std::shared_ptr<action::move>> wall;
					for(auto it = ids_.begin() ; it!=ids_.end() ; ++it){
						wall.push_back(std::make_shared<action::move>(*it));
					}
      		...
      		y_     = ratio * ball_y;
					...
					if(std::signbit(ball_x) == std::signbit(goal_x*(-1)) ){//ボールは敵陣地なのでキーパーはさがる:A
						//ゴール直前でボールに併せて横移動
						
						keeper->move(goal_x , ball_y , util::wrap_to_2pi(std::atan2(0,goal_x-ball_x) + pi<double>()));
					
					}else if(std::signbit(std::pow(ball_x-world_.field().x_max(),2)+std::pow(ball_y,2)-std::pow(demarcation,2))){//ボールは縄張りのなかなので頑張れキーパー:C
						//ゴール前で張ってるキーパの位置
						length = std::hypot(x_ - ball_x, y_ - ball_y); //基準点<->ボール
						ratio = (180) / length;//全体に対してのキーパー位置の比
      			
						keeper->move_to( (1 - ratio) * x_ + ratio * tmp_x , (1 - ratio) * y_ + ratio * tmp_y , theta );//置く場所をセット
