...
std::vector<std::shared_ptr<action::base>> setplay::execute() {
  ...
  for (auto enemyrobot_p : our_robots) {
    const auto enemyrobot = std::get<1>(enemyrobot_p);
    Eigen::Vector2i point((enemyrobot.x() + 4500) / gap.x(), (enemyrobot.y() + 3000) / gap.y());
    std::cout << point.x() << ", " << point.y() << std::endl;
    if (point.x() >= split.x()) point.x() = split.x() - 1;
    if (point.x() < 0) point.x()          = 0;
    if (point.y() >= split.y()) point.y() = split.y() - 1;
  ...
  /* 密集度マトリクスの表示 */
  std::cout << "\e[0;0f\e[0J" << std::endl;
  for (int j = split.y() - 1; j >= 0; j--) {
    for (int i = 0; i < split.x(); i++) {
      std::cout << "\e[" << 30 + (enemydata(i, j) == 2 ? 7 : enemydata(i, j)) << "m"
                << enemydata(i, j);
    }
    std::cout << "\n";
  }
  ...
  if (receiver_ids_.size() == 0) {
    ...
    switch (state_) {
      case setplay::state::setup: {
        ...
        if (positions.size() == 0) {
          ...
          for (auto receiver_id : receiver_ids_) {
            ...
            if (std::abs(ball.x() - enemygoal) < 3000) {
              ...
              pos_ = pos::near;
              positions.push_back(
                  {ball.x() + enemygoalsign * (i + 1) * 1000, std::sin(theta) * 2000, dire});
              std::cout << ball.x() + enemygoalsign * (i + 1) * 1000 << std::sin(theta) * 2000
                        << std::endl;
            } else {
              ...
              std::cout << ball.x() + enemygoalsign * i * 1000 << std::sin(theta) * 2000
                        << std::endl;
            }
            i++;
          }
        ...
        if (movedflag) {
          ...
          for (i = 0; i < static_cast<double>(receiver_ids_.size()); i++) {
            ...
            std::cout << "st1 " << receiver_ids_[i] << std::endl;
          ...
          if (std::hypot(ball.x() - kicker.x(), ball.y() - kicker.y()) > 500) {
            ...
            std::cout << "st2 " << kicker_id_ << std::endl;
            baseaction.push_back(move);
          } else {
            baseaction.push_back(make_action<action::no_operation>(kicker_id_));
            std::cout << "st3 " << kicker_id_ << std::endl;
          }
          break;
        }
      }
      ...
      case state::pass: {
        ...
        for (auto receiver_id : receiver_ids_) {
          ...
          if (receiver_id != shooter_id_) {
            ...
            std::cout << "st6 " << receiver_id << std::endl;
            baseaction.push_back(move);
          } else {
            ...
            std::cout << "st7 " << receiver_id << std::endl;
            baseaction.push_back(move);
          }
        }
        break;
            ...
            if (std::hypot(receiver.x() - kicker.x(), receiver.y() - kicker.y()) >
              ...
              if (receiver_id == shooter_id_) {
                if (pos_ == pos::near &&
                  ...
                  std::cout << "st9 " << receiver_id << std::endl;
                  baseaction.push_back(move);
                } else {
                  double max_dist = 0;
                  double target;
                  ...
                  if (std::hypot(ball.x() - receiver.x(), ball.y() - receiver.y()) < 1000) {
                    ...
                    kick_->set_kick_type({model::command::kick_type_t::line, power});
                    std::cout << "st10 " << kicker_id_ << std::endl;
                    baseaction.push_back(kick_);
                  } else {
                    ...
                    auto move          = make_action<action::move>(shooter_id_);
                    move->move_to(target.x(), target.y(), theta);
                    std::cout << "st11 " << kicker_id_ << std::endl;
                    baseaction.push_back(move);
                  }
                }
              } else {
                std::cout << "st12 " << receiver_id << std::endl;
                baseaction.push_back(make_action<action::no_operation>(receiver_id));
              }
            } else {
              const double power = 100;
              ...
              kick_->set_anglemargin(0.03);
              kick_->set_kick_type({model::command::kick_type_t::line, power});
              std::cout << "st13 " << kicker_id_ << std::endl;
              baseaction.push_back(kick_);
            }
          } else {
            std::cout << "st14 " << receiver_id << std::endl;
            baseaction.push_back(make_action<action::no_operation>(receiver_id));
          }
        }
        break;
      ...
      case state::finished: {
        ...
        std::cout << "st15 " << kicker_id_ << std::endl;
        baseaction.push_back(make_action<action::no_operation>(kicker_id_));
        for (auto receiver_id : receiver_ids_) {
          std::cout << "st16 " << receiver_id << std::endl;
          baseaction.push_back(make_action<action::no_operation>(receiver_id));
        }
        break;
      }
