...
std::vector<std::shared_ptr<action::base>> all::execute() {
  ...
  // フィールドから出られる距離
  constexpr double field_mergin = 200.0;
  // ペナルティエリアから余裕を持たせる距離
  constexpr double penalty_mergin = 150.0;
  // 一般障害物設定
  planner::obstacle_list common_obstacles;
  {
    ...
    common_obstacles.add(model::obstacle::enemy_penalty_area(world().field(), penalty_mergin));
    common_obstacles.add(model::obstacle::our_penalty_area(world().field(), penalty_mergin));
  }
  ...
  // waiter ///////////////////////////
  {
    ...
    for (const auto id : waiters_) {
      ...
      if (pos.x() < wf.x_min() + wf.penalty_length() + penalty_mergin &&
          std::abs(pos.y()) < wf.penalty_width() / 2.0 + penalty_mergin) {
        // 自陣ゴール前で防御
