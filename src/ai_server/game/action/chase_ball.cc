...
model::command chase_ball::execute() {
  ...
  double ball_vx_ = world_.ball().vx();
  double ball_vy_ = world_.ball().vy();

if (std::hypot(ball_vx_,ball_vy_) < 4000) {
  ball_vx = ball_vx_;
  ball_vy = ball_vy_;
}
  ...
  if (mode_ == mode::wait_ball) {
    if (std::hypot(ball_vx, ball_vy) < 300 || std::abs(util::wrap_to_pi(pi<double>() + v1_theta - std::atan2(ball_vy, ball_vx))) > 90 * pi<double>() / 180) {
...
/*  else {
    if (std::hypot(ball_vx,ball_vy) >= 1000) {
      if (std::abs(util::wrap_to_pi(pi<double>() + v1_theta - std::atan2(ball_vy, ball_vx))) < 30 * pi<double>() / 180) {
  ...
  */
  ...
  // 位置の決定
  if (wait_flag_) {
    first_pos = {ball_x_ + start_dist_ * std::cos(std::atan2(ball_vy, ball_vx)), ball_y_ + start_dist_ * std::sin(std::atan2(ball_vy, ball_vx)), pi<double>() + std::atan2(ball_vy, ball_vx)};
  }
  else if (wrap_flag_) {
    if (sign_flag_) {
      first_pos = {ball_x + ball_to_r * std::sin(v1_theta), ball_y - ball_to_r * std::cos(v1_theta),
                v2_theta};
    }
    else {
      first_pos = {ball_x - ball_to_r * std::sin(v1_theta), ball_y + ball_to_r * std::cos(v1_theta),
    ...
    first_pos = {ball_x - ball_to_r * std::cos(v2_theta), ball_y - ball_to_r * std::sin(v2_theta),
                v2_theta};
  }

  double move_theta = std::atan2(first_pos.y - my_pos.y, first_pos.x - my_pos.x);        // move_to_ball_角度

  if (std::hypot(ball_vx_,ball_vy_) < 400) {
  second_pos = {ball_x - ball_to_r * std::cos(v2_theta), ball_y - ball_to_r * std::sin(v2_theta),
                v2_theta};
  }
  else {
  second_pos = {ball_x + ball_to_r * std::cos(std::atan2(ball_vy, ball_vx)), ball_y + ball_to_r * std::sin(std::atan2(ball_vy, ball_vx)), std::atan2(ball_vy, ball_vx) + pi<double>()};
  }
    ...
    case mode::move_to_ball:
      ...
      if (count_ <= acc_count && dist1 > start_dist_ / 2) {
        next_vel = {acc * count_ / 60 * std::cos(move_theta),
                    acc * count_ / 60 * std::sin(move_theta),
                    move_omega};
      }

      else if (count_ > acc_count && dist1 >= acc_dist - 100) {
        next_vel = {max_vel * std::cos(move_theta),
                    max_vel * std::sin(move_theta),
                    move_omega};
      }
 ...
 //位置調整
              ...
              next_vel = {(first_pos.x - my_pos.x) / a,
                          (first_pos.y - my_pos.y) / a,
                          move_omega};
            } else {
              next_vel = {0, 0, 0};
              mode_    = mode::dribble;
    ...
    case mode::wraparound:
      ...
      if (count_ <= acc_count) {
        next_vel = {(second_pos.x - my_pos.x) / (acc_count - count_), (second_pos.y - my_pos.y) /  (acc_count - count_), wrap_angle};
        //next_vel = {rot_vel * std::cos(v2_theta - rot_omega / (acc_count -  count_)) / count_, rot_vel * std::sin(v2_theta - rot_omega / (acc_count -  count_)) / count_, wrap_omega};
      }
      //位置調整
      if (dist2 > 50) {
        next_vel = {(second_pos.x - my_pos.x) / a, (second_pos.y - my_pos.y) / a,
                    wrap_angle};
      } else {
        next_vel = {0, 0, 0};
        mode_    = mode::dribble;
...
/*    else {
      next_vel = {300 * std::cos(std::atan2(target_y_ - my_pos.y, target_x_ - my_pos.x)), 300 * std::sin(std::atan2(target_y_ - my_pos.y, target_x_ - my_pos.x)), wrap_angle};
...
*/
    ...
    case mode::wait_ball:
      ...
      else if (count_ > acc_count && dist1 >= 300) {
        next_vel = {max_vel * std::cos(move_theta),
                    max_vel * std::sin(move_theta),
                    move_omega};
      }
      ...
      // 位置調整
        ...
        next_vel = {(first_pos.x - my_pos.x) / a, (first_pos.y - my_pos.y) / a,
                    move_omega};
      }
      break;
  }
