...
grsim::grsim(boost::asio::io_service& io_service, const std::string& grsim_addr, short port) : udp_sender_(io_service, grsim_addr, port) {}
...
void grsim::send_command(const model::command& command) {
  ...
  if (std::get<0>(command.kick_flag()) == model::command::kick_type_t::chip || std::get<0>(command.kick_flag()) == model::command::kick_type_t::backspin)
    grcommand->set_kickspeedz(static_cast<float>(std::get<1>(command.kick_flag())));

  grcommand->set_veltangent(static_cast<float>(
      boost::get<model::command::velocity_t>(command.setpoint()).vx));
  grcommand->set_velnormal(static_cast<float>(
      boost::get<model::command::velocity_t>(command.setpoint()).vy));
  grcommand->set_velangular(static_cast<float>(
      boost::get<model::command::velocity_t>(command.setpoint()).omega));
